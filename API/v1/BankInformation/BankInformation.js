...
module.exports = function (app) {//MODIFY
  app.get('/Api/v1/BankInformation/Update/:BankInformationID/:UserAccountID/:BankName/:SecurityCode/:Expiration/:Time/:Date', Security.rateLimiterMiddleware, function (req, res) {
...
//INSERT
  app.get('/Api/v1/BankInformation/Add/:UserAccountID/:BankName/:SecurityCode/:Valid/:Expiration/:Time/:Date', Security.rateLimiterMiddleware, function (req, res) {
...
//SELECTION
  app.get('/Api/v1/BankInformation/', Security.rateLimiterMiddleware, function (req, res) {
  ...
  app.get('/Api/v1/BankInformation/Describe', Security.rateLimiterMiddleware, function (req, res) {
    res.setHeader('Content-Type', 'application/json');
    Models.BankInformation.sync( /*{alter:true}*/ ); //Never call Alter and force during a sequelize.query alter table without matching the model with the database first if you do records will be nulled alter is only safe when it matches the database
...
//STRUCTURE
  app.get('/Api/v1/BankInformation/Clear', Security.rateLimiterMiddleware, function (req, res) {
  ...
  app.get('/Api/v1/BankInformation/Delete', Security.rateLimiterMiddleware, function (req, res) {
    Models.BankInformation.sync({
      force: true
    }).then(function (result) {
      res.send("Deleted");
