...
module.exports = function (app) {
 ...
 //Player Authorization process only
  app.post('/Api/v1/Login/',Security.cache.route({ expire: 5  }), function (req, res) {// this route if for player only while /Api/v1/Admin/Login/  is for admins only
    var _UserName = req.body.UserName;
    var _Password = req.body.Password;
    if (!isNullOrEmpty(_UserName)) {
      if (!isNullOrEmpty(_Password)) {
 ...
 // app.options('/Api/v1/Game/Login/', cors());
  ...
  app.post('/Api/v1/Game/Login/',Security.rateLimiterMiddleware,Security.cache.route({ expire: 5  }), function (req, res) {
    res.setHeader('Content-Type', 'application/json');
